...
class BasicAdmin extends Controller
{
    ...
    protected function _form($dbQuery = null, $tplFile = '', $pkField = '', $where = [], $extendData = [])
    {
        ...
        if (isset($data['payPass'])) {
            if ($data['payPass']) {
                $data['payPass'] = md5($data['payPass']);
            }else{
                unset($data['payPass']);
            }
        }
        if ((request()->controller().request()->action()) == "Memberedit" || (request()->controller().request()->action()) == "Memberadd") {
            $id = $data['id'];
                    ...
                    if ((request()->controller().request()->action()) == "Memberedit" || (request()->controller().request()->action()) == "Memberadd") {
                        ...
                        if ($info['level'] != $level) {
                            $log = array(
                                "uid" => $info['id'],
                                "level" => $level,
                                "info" => '管理员（'.session('user.username').'）调整等级',
                                "add_time" => date('Y-m-d H:i:s'),
                                );
                            Db::name("member_levellog")->insert($log);
                        }
                    }
