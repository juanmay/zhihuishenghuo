 ...
 */
class Frepayment extends BasicAdmin
{
    ...
    {
        // 余额代还进行中数据库对象
        list($this->title, $get) = ['待审核计划', $this->request->get()];
        ...
        ->join('credit_card c','a.bank_id=c.id','left')
        ->field('a.*,b.realname,b.phone,c.bank_num,c.bank_name')
        ->order('a.id DESC');
        ...
        return parent::_list($db->where('a.status','in','4')->where('isPay',1));
    }
     /**
     * 已完成列表
     * @return array|string
    ...
    {
        // 余额代还进行中数据库对象
        list($this->title, $get) = ['已冻结计划', $this->request->get()];
        ...
        return parent::_list($db->where('a.status','in','0,1,2,3'));
    }
