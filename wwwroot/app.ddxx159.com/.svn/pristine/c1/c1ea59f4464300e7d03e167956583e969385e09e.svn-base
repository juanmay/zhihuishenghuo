 ...
 */
class Achievement extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        $db = Db::connect($db_config)->name($this->table)->alias('m')
            ->join('achievement a','a.uid = m.id','left')
            ->field('m.id,m.avatar,m.phone,m.realname,m.recommendId,m.level,sum(a.repayment_money) as money,sum(a.reward_money) as reward')
            ->group("m.id")
