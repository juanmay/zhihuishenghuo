	...
	function getSignMsg($pay_params = array(), $sign_type,$_input_charset) {
		...
		switch (@$sign_type) {
			case 'RSA' :
				...
				$priv_key = file_get_contents ( rsa_sign_private_key );
...
//				self::write_log("MD5计算得出签名值：".$signMsg);
	...
	function Rsa_encrypt($data, $_input_charset) {
		...
		$cert = file_get_contents (rsa_public_key );
	...
	function checkSignMsg($pay_params = array(), $sign_type,$_input_charset) {
		...
		switch (@$sign_type) {
			case 'RSA' :
				$cert = file_get_contents ( rsa_sign_public_key );
	...
	function curlPost($url, $data,$_input_charset) {
		...
		curl_setopt ($ch, CURLOPT_TIMEOUT, 30);//超时30秒关闭，避免报错
		$data = curl_exec ( $ch );
		curl_close ( $ch );
		if($data === false){
			//构造-酷宝-公共返回参数信息
			$result_data=array(
					"response_time" =>date('YmdHis',time()),
					"response_message" => curl_error($ch),
					"sign_type" =>'RSA',
					"response_code" =>'YINYUAN_CUSTOM_TIMEOUT',
			);
			self::write_log("请求网关返回内容:".mb_convert_encoding(urldecode($result_data),"UTF-8"));
 			return $result_data;
		}else{
