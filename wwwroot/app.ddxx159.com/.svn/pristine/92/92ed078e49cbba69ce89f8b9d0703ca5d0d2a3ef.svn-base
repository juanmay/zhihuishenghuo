 ...
 */
class Levellog extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        $db = Db::connect($db_config)->name($this->table)->alias('a')
            ...
            ->join('member_level l','l.id = a.level','left')
            ->field('m.id,m.avatar,m.phone,m.realname,m.recommendId,l.name as level,a.info,a.add_time')
            ->order('a.id desc')->where('m.dataFlag',1);
        // 会员信息查询过滤
