...
class User extends BasicAdmin
{
    ...
    {
        $this->title = '系统用户管理';
        list($get, $db) = [$this->request->get(), Db::name($this->table)->where('id','>',10003)];
        ...
        if(session('user.isAllShow') == '0'){
            $db->where(['fid'=>session('user.id')]);
        }
        return parent::_list($db->where(['dataFlag' => '1']));
    }
    ...
    public function _form_filter(&$data)
    {
        if ($this->request->isPost()) {
            ...
            $this->assign('authorizes', Db::name('SystemAuth')->where(['status' => '1','dataFlag'=>"1"])->where('id','>',1)->select());
        }
    }
