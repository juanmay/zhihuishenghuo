...
class DataService
{
    ...
    public static function update(&$dbQuery, $where = [])
    {
        ...
        // 删除模式，如果存在 dataFlag 字段使用软删除
        if ($field === 'delete') {
            if (method_exists($db, 'getTableFields') && in_array('dataFlag', $db->getTableFields())) {
                return Db::table($table)->where($where)->where($map)->update(['dataFlag' => '0']) !== false;
            }
