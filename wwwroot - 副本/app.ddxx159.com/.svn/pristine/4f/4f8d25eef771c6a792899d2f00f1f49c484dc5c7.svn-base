<?php /*a:2:{s:75:"C:\phpStudy\PHPTutorial\WWW\dscjall\application\admin\view\group\money.html";i:1560849961;s:78:"C:\phpStudy\PHPTutorial\WWW\dscjall\application\admin\view\public\content.html";i:1556184467;}*/ ?>
<!-- 右则内容区域 开始 -->
...
<form autocomplete="off" class="layui-form layui-form-pane form-search" action="<?php echo request()->url(); ?>" onsubmit="return false" method="get">
    ...
    <?php if (input('id')>0): ?>
    <div class="layui-form-item layui-inline">
        <button class="layui-btn layui-btn-primary" onclick="javascript:location.href='<?php echo URL("/admin.html#/admin/group/money"); ?>?allmemberId=<?php echo htmlentities($allmemberId); ?>&spm=<?php echo input('spm'); ?>';"><i class="layui-icon">&#xe603;</i> 返 回</button>
    </div>
    <?php endif ?>
<script>
    window.form.render();
    ...
    <table class="layui-table" lay-size="sm">
        ...
        <tr>
            ...
            <td class='text-center notselect'>
                <a href='<?php echo URL("/admin.html#/admin/group/money"); ?>?spm=<?php echo input('spm'); ?>&id=<?php echo htmlentities($vo['id']); ?>&allmemberId=<?php echo htmlentities($allmemberId); ?>'>查看名下用户</a>
            </td>
        </tr>
        <?php endforeach; ?>
        </tbody>
